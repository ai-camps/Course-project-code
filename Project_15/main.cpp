// **********************************
// * Project 15
// * based from project 14, connect MCU to Amazon Web Services (AWS) IoT Core through MQTT protocols
// **********************************
// * Import libraries
...
void setup()
{
    ...
    deviceID = String(ESP.getEfuseMac(), HEX); // Get the device ID from the ESP32's MAC address
    ...
    if (!connectAWS())
    {
        Serial.println("Failed to connect to AWS Cloud: " + String(AWS_IOT_MQTT_SERVER));
        display.clearDisplay();
        display.setCursor(0, 0);
        display.println("Failed to connect to AWS Cloud: " + String(AWS_IOT_MQTT_SERVER));
        display.print("ESP32 is rebooting in ");
        display.print(RESTART_DELAY_MS / 1000);
        display.println(" seconds.");
        display.display();
...
bool connectToWiFi()
{
    ...
    while (attempt < maxRetries)
    {
        ...
        if (WiFi.status() == WL_CONNECTED)
        {
            ...
            display.clearDisplay();
            display.setCursor(0, 0);
            display.println("WiFi is connected successfully.");
            display.display();
...
bool readAndDisplayTiltSensor() {
    ...
    display.println(tiltState ? "Yes" : "No");
    display.display();
    return tiltState;
}
...
bool readAndDisplayMotionSensor() {
    ...
    display.println(motionState ? "Yes" : "No");
    display.display();
    return motionState;
}
...
bool readAndDisplayFlameSensor() {
    ...
    display.println(flameState ? "Yes" : "No");
    display.display();
    return flameState;
}
