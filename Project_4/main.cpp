// **********************************
// * Project 4
// * Connect a buzzer on MCU Pin GPIO-1 and beep it when Wi-fi access is successful
// **********************************
// * Libraries
...
void syncNTP(); // Declares the function that will synchronize the NTP time
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    digitalWrite(BUZZER_PIN, HIGH); // Turn on the buzzer when WiFi is connected
    Serial.println("LED is On!"); // Print the message when the LED is on
    Serial.println("Buzzer is On!"); // Print the message when the buzzer is on
    ...
    digitalWrite(BUZZER_PIN, LOW); // Turn off the buzzer when WiFi is not connected
  }
