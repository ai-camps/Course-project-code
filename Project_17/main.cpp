...
void loop()
{
    ...
    if (currentTime - lastUpdate >= updateInterval)
    {
        ...
        Serial.println("AWS Connection Status: " + String(awsConnected ? "Connected" : "Disconnected")); 
        ...
        if (!mqttClient.connected())
        {
            ...
            if (!connectAWS())
            {
                Serial.println("Rebooting in 10 seconds due to AWS connection failure...");
                display.clearDisplay();
                display.setCursor(0, 0);
                display.println("AWS connection failed");
                display.println("Rebooting in 10 seconds...");
                display.display();
                ledcWrite(redLedChannel, 255);      
                delay(REBOOT_DELAY_SECONDS * 1000); 
...
{
    if (awsConnected) {
        return true; // If already connected, no need to reconnect
    }
    ...
    while (!mqttClient.connect(deviceID.c_str()) && attempt < 3)
    {
        ...
        display.print("/3 failed, retrying...");
